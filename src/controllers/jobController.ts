...
import Job, { IJob as IJob } from '../models/jobModel.js';
...
export const getJobs = async (
  ...
  try {
    ...
    let maxRev: number, minRev: number;
...
// @access    Private
export const updateJobView = async (req: Request, res: Response) => {
  try {
    const { clientId, jobId } = req.params;
    const account = req.user?.account;
    const c = await Client.findOne({ _id: clientId, account });
...
// @access    Private
export const createJobNote = async (req: Request, res: Response) => {
  try {
    const { clientId, jobId } = req.params;
    const account = req.user?.account;
